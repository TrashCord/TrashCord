/*
 * TrashCord, a Discord client mod
...
let arApplyPromise: Promise<void> = Promise.resolve();
...
const WX_EMOJI: Record<number, string> = {
    113:"☀️",116:"⛅",119:"☁️",122:"☁️",143:"🌫️",176:"🌦️",179:"🌨️",182:"🌧️",185:"🌧️",
    200:"⛈️",227:"🌨️",230:"❄️",248:"🌫️",260:"🌫️",263:"🌦️",266:"🌦️",281:"🌧️",284:"🌧️",
    293:"🌧️",296:"🌧️",299:"🌧️",302:"🌧️",305:"🌧️",308:"🌧️",311:"🌧️",314:"🌧️",317:"🌨️",
    320:"🌨️",323:"🌨️",326:"🌨️",329:"❄️",332:"❄️",335:"❄️",338:"❄️",350:"🌨️",353:"🌧️",
    356:"🌧️",359:"🌧️",362:"🌨️",365:"🌨️",368:"🌨️",371:"❄️",374:"🌨️",377:"🌨️",
    386:"⛈️",389:"⛈️",392:"⛈️",395:"⛈️",
};

interface WxCache { emoji: string; tempC: string; tempF: string; city: string; condition: string; fetchedAt: number; }
...
function bcrFavsAvgHue(): number | null {
    if (!bcrFavorites.length) return null;
    let x = 0, y = 0;
    for (const hex of bcrFavorites) {
        const rad = bcrHexToHsv(hex)[0] * Math.PI / 180;
...
async function bcrGetAvatarHue(fallback?: number): Promise<number> {
    if (bcrCachedHue !== null) return bcrCachedHue;
    const rnd = () => fallback ?? Math.random() * 360;
    try {
        const me = await RestAPI.get({ url: "/users/@me" });
        const { avatar, id } = me?.body ?? {};
        if (!avatar || !id) return rnd();
...
function bcrBuildShadeSequence(lightFirst: boolean): string[] {
    ...
    for (let i = 0; i < steps; i++) { const t = i / (steps - 1); const l = lightFirst ? 88 - t * 83 : 5 + t * 83; seq.push(bcrHslToHex(h, s * 100, l)); }
    return seq;
}
async function bcrPickNextColor(): Promise<string> {
    ...
    if (mode === "mono_cycle") {
        if (bcrMonoBaseHue === null || !bcrRandomBatch.length) { const [h] = bcrGetBaseHsv(); bcrMonoBaseHue = h; bcrRandomBatch = bcrSfShuffle(bcrBuildBatch(32, () => bcrHslToHex(bcrMonoBaseHue!, 25 + Math.random() * 70, 10 + Math.random() * 75))); }
        return bcrRandomBatch.shift()!;
    }
    ...
    if (mode === "favorites_hue") { if (bcrFavorites.length && Math.random() < 0.5) return bcrPickFromFavs(); if (!bcrRandomBatch.length) { const hue = await bcrGetAvatarHue(bcrFavsAvgHue() ?? undefined); bcrRandomBatch = bcrSfShuffle(bcrBuildBatch(32, () => bcrHslToHex(hue + (Math.random() * 2 - 1) * R, 40 + Math.random() * 55, 20 + Math.random() * 50))); } return bcrRandomBatch.shift()!; }
    if (mode === "favs_shade") { const base = bcrFavorites.length ? bcrPickFromFavs() : bcrRandomHex(); const [hF, sF, vF] = bcrHexToHsv(base); const lF = vF * 100; return bcrHslToHex(hF, sF * 100, Math.max(8, Math.min(88, lF + (Math.random() * 2 - 1) * 40))); }
    return bcrRandomHex();
}
...
function bcrSchedule() { if (bcrRotatorTimer) clearTimeout(bcrRotatorTimer); if (!pluginActive || isManualStop || wasInvisible || bcrNitroBlocked()) return; const ms = Math.max(1, settings.store.bannerIntervalSeconds ?? BCR_DEFAULT_S) * 1000; bcrRotatorTimer = setTimeout(bcrRotateNext, netGuardMultiplier > 1 ? ms * netGuardMultiplier : ms); }
...
let netGuardMultiplier = 1;
let netGuardPaused = false;
let _netGuardFallbackInterval: ReturnType<typeof setInterval> | null = null;
let _netHighCount = 0;

let _consolePatched = false;
let _origConsoleErr: typeof console.error | null = null;
let _origConsoleWarn: typeof console.warn | null = null;
let _consoleErrCount = 0;
let _consoleClearTimer: ReturnType<typeof setInterval> | null = null;
let _consoleErrHandler: ((e: Event) => void) | null = null;
let _consoleRejHandler: ((e: Event) => void) | null = null;
...
function getMs(sec: string): number { const base = Math.max(1000, (parseFloat(sec) || 30) * 1000); return netGuardMultiplier > 1 ? base * netGuardMultiplier : base; }
...
function scheduleNickTick(g: GuildEntry, ms: number) {
    const id = setTimeout(async () => {
        if (nickTimers.get(g.id) !== id) return;
        nickTimers.delete(g.id);
        if (!pluginActive || !g.enabled || !settings.store.nickEnabled) return;
        ...
        if (nick) {
            ...
            if (retry !== null) {
                scheduleNickTick(g, retry * 1000 + 429);
                return;
            }
        }
        saveData();
        if (!settings.store.globalSync && pluginActive && g.enabled && settings.store.nickEnabled && !nickTimers.has(g.id))
            scheduleNickTick(g, getMs(settings.store.nickIntervalSeconds));
    }, ms);
    nickTimers.set(g.id, id);
}
...
async function resolvePreviewField(value: string): Promise<{ out: string; err: boolean }> {
    if (!value.startsWith(RS_EVAL)) return { out: value, err: false };
    try {
        const _eval = globalThis.eval;
        const result = _eval(value.slice(RS_EVAL.length));
        return { out: String(result ?? ""), err: false };
    } catch (e: any) {
        return { out: e?.message ?? "eval error", err: true };
    }
}
...
function startAllRotators() {
    stopAllRotators();
    if (!pluginActive) return;
    if (isManualStop || wasInvisible || netGuardPaused) return;
    ...
    if ((settings.store as any).weatherEnabled) wxStart();
}
...
function _netGetRtt(): number {
    try {
        const btn = document.querySelector('[class*="clickablePing"]') as HTMLElement | null;
        if (btn) {
            const id = btn.getAttribute("aria-describedby");
            const span = id ? document.getElementById(id) : null;
            if (span) {
                const ms = parseInt(span.textContent ?? "");
                if (ms > 0) return ms;
            }
        }
    } catch {}
    try {
        ...
        if (c && typeof c.rtt === "number" && c.rtt > 0) return c.rtt;
    } catch {}
    return 0;
}

function _netApply() {
    if (!pluginActive || !(settings.store as any).netGuardEnabled) return;
    const rtt = _netGetRtt();
    const th = Math.max(50, Number((settings.store as any).netGuardRttThreshold) || 200);

    if (rtt === 0 || rtt <= th) {
        _netHighCount = 0;
        if (netGuardPaused) {
            netGuardPaused = false;
            netGuardMultiplier = 1;
            if (!isManualStop && !wasInvisible) startAllRotators();
        } else if (netGuardMultiplier > 1) {
            netGuardMultiplier = 1;
            if (!isManualStop && !wasInvisible) startAllRotators();
        }
    } else {
        _netHighCount = Math.min(_netHighCount + 1, 10);
        if (_netHighCount < 2) return;
        if (rtt > th * 2.5) {
            if (!netGuardPaused) {
                netGuardPaused = true;
                netGuardMultiplier = 0;
                stopAllNicks(); stopAllGuildPronouns();
                stopStatusTimer(); stopClanTimer();
                stopBioTimer(); stopPronounsTimer(); stopGlobalNickTimer();
                stopGlobalTimer(); stopVoiceWatcher(); bcrStopRotator(); arStopRotator();
            ...
            const newM = rtt > th * 1.5 ? 5 : 3;
            if (!netGuardPaused && netGuardMultiplier !== newM) {
                netGuardMultiplier = newM;
            }
        }
    }
}

function stopNetGuard() {
    if (_netGuardFallbackInterval) { clearInterval(_netGuardFallbackInterval); _netGuardFallbackInterval = null; }
    try {
        const c = (navigator as any).connection;
        if (c && (c as any)._rsNG) { c.removeEventListener("change", (c as any)._rsNG); delete (c as any)._rsNG; }
    } catch {}
    if (netGuardPaused || netGuardMultiplier !== 1) { netGuardPaused = false; netGuardMultiplier = 1; }
    _netHighCount = 0;
}

function startNetGuard() {
    if (!(settings.store as any).netGuardEnabled) return;
    stopNetGuard();
    _netApply();
    _netGuardFallbackInterval = setInterval(() => { if (pluginActive) _netApply(); }, 4000);
    try {
        const c = (navigator as any).connection;
        if (c) { (c as any)._rsNG = () => { if (pluginActive) _netApply(); }; c.addEventListener("change", (c as any)._rsNG); }
    } catch {}
}

function _consoleOnHit() {
    _consoleErrCount++;
    const th = Math.max(50, Number((settings.store as any).consoleCleanThreshold) || 500);
    if (_consoleErrCount >= th) { _consoleErrCount = 0; console.clear(); }
}

function stopConsoleCleaner() {
    if (_consoleClearTimer) { clearInterval(_consoleClearTimer); _consoleClearTimer = null; }
    if (_consoleErrHandler) { window.removeEventListener("error", _consoleErrHandler, true); _consoleErrHandler = null; }
    if (_consoleRejHandler) { window.removeEventListener("unhandledrejection", _consoleRejHandler, true); _consoleRejHandler = null; }
    if (_consolePatched) {
        _consolePatched = false;
        if (_origConsoleErr) { console.error = _origConsoleErr; _origConsoleErr = null; }
        if (_origConsoleWarn) { console.warn = _origConsoleWarn; _origConsoleWarn = null; }
    }
    _consoleErrCount = 0;
}

function startConsoleCleaner() {
    if (!(settings.store as any).consoleCleanerEnabled) return;
    stopConsoleCleaner();
    _consoleErrHandler = () => _consoleOnHit();
    _consoleRejHandler = () => _consoleOnHit();
    window.addEventListener("error", _consoleErrHandler, true);
    window.addEventListener("unhandledrejection", _consoleRejHandler, true);
    if (!_consolePatched) {
        _consolePatched = true;
        _origConsoleErr = console.error;
        _origConsoleWarn = console.warn;
        const oe = _origConsoleErr, ow = _origConsoleWarn;
        console.error = function(...a: any[]) { _consoleOnHit(); oe.apply(console, a); };
        console.warn = function(...a: any[]) { _consoleOnHit(); ow.apply(console, a); };
    }
}
...
const settings = definePluginSettings({
    ...
    _sNetGuard: { type: OptionType.COMPONENT, description: "", component: () => <SettingsSep title="Network Guard" color="#26c6da" /> },
    netGuardEnabled: { type: OptionType.BOOLEAN, default: true, description: "Throttle or pause rotators when RTT is high (bufferbloat protection)." },
    netGuardRttThreshold: { type: OptionType.NUMBER, default: 200, description: "RTT ms threshold. Above this → slow ×3/×5. Above 2.5× → full pause." },
    _sConsoleCleaner: { type: OptionType.COMPONENT, description: "", component: () => <SettingsSep title="Console Cleaner" color="#66bb6a" /> },
    consoleCleanerEnabled: { type: OptionType.BOOLEAN, default: true, description: "Intercept console.error/warn and auto-clear after N entries to prevent Discord crash." },
    consoleCleanThreshold: { type: OptionType.NUMBER, default: 500, description: "Error+warn count before threshold-clear (min 50)." },
    _sWeather: { type: OptionType.COMPONENT, description: "", component: () => <SettingsSep title="Weather" color="#64b5f6" /> },
    weatherEnabled: { type: OptionType.BOOLEAN, default: false, description: "Enable weather data polling (used in eval and standalone status)." },
    weatherCity: { type: OptionType.STRING, default: "", description: "City for weather data (e.g. Rome, New York, Tokyo)." },
    weatherUnit: { type: OptionType.STRING, default: "C", description: "Temperature unit: C or F." },
    weatherIntervalMin: { type: OptionType.NUMBER, default: 15, description: "Minutes between weather data refreshes." },
    weatherStandaloneEnabled: { type: OptionType.BOOLEAN, default: false, description: "Apply weather as standalone Discord status (independent from rotator)." },
    weatherTemplate: { type: OptionType.STRING, default: "{emoji} {temp} · {city}", description: "Standalone weather status template. Tokens: {emoji} {temp} {city} {condition}" },
});

async function wxFetch(city: string): Promise<WxCache | null> {
    try {
        const res = await fetch(`https://wttr.in/${encodeURIComponent(city)}?format=j1`, { headers: { "Accept": "application/json" } });
        if (!res.ok) return null;
        const data = await res.json();
        const c = data?.current_condition?.[0];
        if (!c) return null;
        const code = parseInt(c.weatherCode, 10);
        return { emoji: WX_EMOJI[code] ?? "🌡️", tempC: c.temp_C, tempF: c.temp_F, city, condition: c.weatherDesc?.[0]?.value ?? "", fetchedAt: Date.now() };
    } catch { return null; }
}

function wxBuildText(cache: WxCache, template: string, useF: boolean): string {
    return template
        .replace(/\{emoji\}/g, cache.emoji)
        .replace(/\{temp\}/g, useF ? `${cache.tempF}°F` : `${cache.tempC}°C`)
        .replace(/\{city\}/g, cache.city)
        .replace(/\{condition\}/g, cache.condition)
        .slice(0, 128);
}

async function wxUpdate() {
    if (!pluginActive) return;
    const city = (settings.store as any).weatherCity?.trim();
    if (!city) return;
    const result = await wxFetch(city);
    if (!result) return;
    wxCache = result;
    (window as any).__rsWx = { emoji: result.emoji, tempC: result.tempC, tempF: result.tempF, city: result.city, condition: result.condition };
    if ((settings.store as any).weatherStandaloneEnabled) {
        const tpl = (settings.store as any).weatherTemplate || "{emoji} {temp} · {city}";
        const text = wxBuildText(result, tpl, (settings.store as any).weatherUnit === "F");
        if (text) void applyStatus({ emojiName: null, emojiId: null, text });
    }
}

function wxSchedule() {
    if (wxTimer) { clearTimeout(wxTimer); wxTimer = null; }
    if (!pluginActive || !(settings.store as any).weatherEnabled) return;
    const ms = Math.max(1, Number((settings.store as any).weatherIntervalMin) || 15) * 60_000;
    wxTimer = setTimeout(() => { wxTimer = null; void wxUpdate().then(() => { if (pluginActive) wxSchedule(); }); }, ms);
}

function wxStart() {
    wxStop();
    if (!(settings.store as any).weatherEnabled) return;
    void wxUpdate().then(() => { if (pluginActive) wxSchedule(); });
}
...
.rs-modal input:not([type="range"]):not([type="file"]),.rs-modal textarea,.rs-modal select{background-color:rgba(10,0,30,.7)!important;border:1px solid rgba(124,77,255,.35)!important;outline:none!important;box-shadow:none!important;appearance:none!important}
.rs-modal input:not([type="range"]):not([type="file"]):focus,.rs-modal textarea:focus,.rs-modal select:focus{border-color:#9c67ff!important}
...
.rs-item-text{flex:1;font-size:12px;color:#f0eaff;cursor:pointer;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.rs-item-text:hover{color:#ce93d8}
.rs-item-mono{flex:1;font-size:12px;font-family:monospace;color:#b0c4de;cursor:pointer;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
...
.rs-status-draft{width:100%;box-sizing:border-box;resize:none;border-radius:6px;font-size:13px;padding:6px 9px;font-family:inherit;line-height:1.4;background:rgba(10,0,30,.85)!important;background-color:rgba(10,0,30,.85)!important;border:1px solid rgba(124,77,255,.45)!important;color:#f0eaff!important;outline:none!important;box-shadow:none!important;appearance:none!important}
.rs-status-draft:focus{border-color:#9c67ff!important}
...
const EVAL_SNIPPETS: { cat: string; items: [string, string][] }[] = [
    { cat: "🕐 Time", items: [
        ["HH:MM:SS", "eval var f=t=>(t<10?'0':'')+t,d=new Date();`${f(d.getHours())}:${f(d.getMinutes())}:${f(d.getSeconds())}`"],
        ["HH:MM", "eval var f=t=>(t<10?'0':'')+t,d=new Date();`${f(d.getHours())}:${f(d.getMinutes())}`"],
    ...
    { cat: "📅 Date", items: [
        ...
        ["DD/MM/YYYY", "eval var d=new Date();`${d.getDate()}/${d.getMonth()+1}/${d.getFullYear()}`"],
        ["DD/MM", "eval var d=new Date();`${d.getDate()}/${d.getMonth()+1}`"],
        ["Month/Year", "eval var d=new Date();`${d.getMonth()+1}/${d.getFullYear()}`"],
    ...
    { cat: "🎲 Fun", items: [
        ...
        ["Magic 8-ball", "eval ['Yes','No','Maybe','Ask later','Definitely','Unlikely'][Math.floor(Math.random()*6)]"],
        ["Slot 🎰", "eval ['🍒','🍋','🔔','⭐','💎','7️⃣'][Math.floor(Math.random()*6)]+['🍒','🍋','🔔','⭐','💎','7️⃣'][Math.floor(Math.random()*6)]+['🍒','🍋','🔔','⭐','💎','7️⃣'][Math.floor(Math.random()*6)]"],
        ["Random flag", "eval ['🇮🇹','🇺🇸','🇬🇧','🇫🇷','🇩🇪','🇯🇵','🇪🇸','🇧🇷'][Math.floor(Math.random()*8)]"],
    ]},
    { cat: "📊 System", items: [
    ...
    { cat: "🌤️ Weather", items: [
        ["Temp °C", "eval (window.__rsWx?.emoji??'?')+' '+(window.__rsWx?.tempC??'?')+'°C'"],
        ["Temp °F", "eval (window.__rsWx?.emoji??'?')+' '+(window.__rsWx?.tempF??'?')+'°F'"],
        ["Wx + City", "eval (window.__rsWx?.emoji??'?')+' '+(window.__rsWx?.tempC??'?')+'°C · '+(window.__rsWx?.city??'?')"],
        ["Wx + Cond", "eval (window.__rsWx?.emoji??'?')+' '+(window.__rsWx?.tempC??'?')+'°C '+(window.__rsWx?.condition??'')"],
        ["Wx+City+Time", "eval var f=t=>(t<10?'0':'')+t,d=new Date();`${window.__rsWx?.emoji??'?'} ${window.__rsWx?.tempC??'?'}°C · ${window.__rsWx?.city??'?'} | ${f(d.getHours())}:${f(d.getMinutes())}:${f(d.getSeconds())}`"],
        ["Full combo", "eval var f=t=>(t<10?'0':'')+t,d=new Date();`${window.__rsWx?.emoji??'?'} ${window.__rsWx?.tempC??'?'}°C · ${window.__rsWx?.city??'?'} | 𖤍 | ${f(d.getHours())}:${f(d.getMinutes())}:${f(d.getSeconds())}`"],
    ]},
];
...
const EVAL_TOKENS: { label: string; code: string; pad: boolean }[] = [
    ...
    { label: "DD/MM", code: "${new Date().getDate()}/${new Date().getMonth()+1}", pad: false },
    { label: "Month name", code: "${['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'][new Date().getMonth()]}", pad: false },
    { label: "Year", code: "${new Date().getFullYear()}", pad: false },
    { label: "Clock emoji", code: "${['🕛','🕐','🕑','🕒','🕓','🕔','🕕','🕖','🕗','🕘','🕙','🕚'][new Date().getHours()%12]}", pad: false },
    { label: "Moon phase", code: "${['🌑','🌒','🌓','🌔','🌕','🌖','🌗','🌘'][Math.floor(new Date().getDate()/3.75)%8]}", pad: false },
    { label: "Random 1-100", code: "${Math.floor(Math.random()*100)+1}", pad: false },
    { label: "Dice 🎲", code: "${['⚀','⚁','⚂','⚃','⚄','⚅'][Math.floor(Math.random()*6)]}", pad: false },
    { label: "Magic 8-ball", code: "${['Yes','No','Maybe','Ask later','Definitely','Unlikely'][Math.floor(Math.random()*6)]}", pad: false },
    { label: "Random flag", code: "${['🇮🇹','🇺🇸','🇬🇧','🇫🇷','🇩🇪','🇯🇵','🇪🇸','🇧🇷'][Math.floor(Math.random()*8)]}", pad: false },
    { label: "Random color", code: "${'#'+Math.floor(Math.random()*0xFFFFFF).toString(16).padStart(6,'0')}", pad: false },
    { label: "🌤 Wx emoji", code: "${window.__rsWx?.emoji??'?'}", pad: false },
    { label: "🌡 Temp °C", code: "${window.__rsWx?.tempC??'?'}°C", pad: false },
    { label: "🌡 Temp °F", code: "${window.__rsWx?.tempF??'?'}°F", pad: false },
    { label: "📍 Wx city", code: "${window.__rsWx?.city??'?'}", pad: false },
    { label: "💬 Wx cond", code: "${window.__rsWx?.condition??'?'}", pad: false },
];
...
function EvalSnippetPanel({ setDraft }: { setDraft: (v: string) => void }) {
    ...
    const [bToken2, setBToken2] = React.useState(-1);
    const [bRight, setBRight] = React.useState("");

    const buildExpr = () => {
        const t1 = EVAL_TOKENS[bToken];
        const t2 = bToken2 >= 0 ? EVAL_TOKENS[bToken2] : null;
        const needsPad = t1.pad || (t2?.pad ?? false);
        const body = `${bLeft}${t1.code}${t2 ? bMid : ""}${t2 ? t2.code : ""}${bRight}`;
        return needsPad ? `eval var f=t=>(t<10?'0':'')+t,d=new Date();\`${body}\`` : `eval \`${body}\``;
    };
    ...
    const selStyle: React.CSSProperties = { background: "rgba(10,0,30,.85)", border: "1px solid rgba(255,167,38,.4)", borderRadius: 5, color: "#ffa726", fontSize: 11, padding: "3px 6px", outline: "none" };
    const txtStyle: React.CSSProperties = { flex: 1, minWidth: 70, background: "rgba(10,0,30,.7)", border: "1px solid rgba(255,167,38,.3)", borderRadius: 5, color: "#f0eaff", fontSize: 11, padding: "3px 7px", outline: "none", fontFamily: "monospace" };
        ...
        <div>
            ...
            <div style={{ marginTop: 6, borderTop: "1px solid rgba(255,167,38,.15)", paddingTop: 5 }}>
                <button onClick={() => setBuilderOpen(o => !o)}
                    ...
                    {builderOpen ? "▾" : "▸"} Text builder - mix text with up to 2 dynamic parts
                </button>
                {builderOpen && (
                    <div style={{ marginTop: 5, display: "flex", flexDirection: "column" as const, gap: 5 }}>
                        <div style={{ fontSize: 10, color: "#9e9e9e" }}>
                            Write text around one or two dynamic parts. Example: <span style={{ fontFamily: "monospace", color: "#faa61a", userSelect: "all" as const }}>🖤 ... | HH:MM:SS ... · ... | Random flag</span>
                        </div>
                        <div style={{ display: "flex", gap: 5, alignItems: "center", flexWrap: "wrap" as const }}>
                            <input value={bLeft} onChange={e => setBLeft(e.target.value)} placeholder="text before..." style={txtStyle} />
                            <select value={bToken} onChange={e => setBToken(Number(e.target.value))} style={selStyle}>
                                {EVAL_TOKENS.map((t, i) => <option key={t.label} value={i}>{t.label}</option>)}
                            </select>
                            {bToken2 >= 0 && (
                                <>
                                    <input value={bMid} onChange={e => setBMid(e.target.value)} placeholder="text between..." style={txtStyle} />
                                    <select value={bToken2} onChange={e => setBToken2(Number(e.target.value))} style={selStyle}>
                                        {EVAL_TOKENS.map((t, i) => <option key={t.label} value={i}>{t.label}</option>)}
                                    </select>
                                </>
                            )}
                            <input value={bRight} onChange={e => setBRight(e.target.value)} placeholder="text after..." style={txtStyle} />
                        </div>
                        <div>
                            {bToken2 >= 0
                                ? <button onClick={() => { setBToken2(-1); setBMid(""); }}
                                    style={{ fontSize: 10, fontWeight: 700, color: "#c8a050", background: "none", border: "1px solid rgba(255,167,38,.25)", borderRadius: 5, padding: "2px 8px", cursor: "pointer", outline: "none" }}>
                                    − Remove 2nd dynamic part
                                </button>
                                : <button onClick={() => setBToken2(bToken === 0 ? 1 : 0)}
                                    style={{ fontSize: 10, fontWeight: 700, color: "#c8a050", background: "none", border: "1px solid rgba(255,167,38,.25)", borderRadius: 5, padding: "2px 8px", cursor: "pointer", outline: "none" }}>
                                    + Add 2nd dynamic part
                                </button>}
                        </div>
...
function StatusLivePreview({ entry, statusType }: { entry: Pick<StatusEntry, "emojiId" | "emojiName" | "animated" | "text">; statusType: StatusType }) {
    ...
    const isEvalText = !!entry.text?.startsWith("eval ");
    const isEvalEmoji = !entry.emojiId && !!entry.emojiName?.startsWith("eval ");
    const [live, setLive] = React.useState<{ text: string; emojiName: string | null; err: boolean }>({ text: entry.text ?? "", emojiName: entry.emojiName ?? null, err: false });

    React.useEffect(() => {
        if (!isEvalText && !isEvalEmoji) {
            setLive({ text: entry.text ?? "", emojiName: entry.emojiName ?? null, err: false });
            return;
        }
        let alive = true;
        const run = async () => {
            const [t, en] = await Promise.all([
                isEvalText ? resolvePreviewField(entry.text!) : Promise.resolve({ out: entry.text ?? "", err: false }),
                isEvalEmoji ? resolvePreviewField(entry.emojiName!) : Promise.resolve({ out: entry.emojiName ?? null, err: false }),
            ]);
            if (alive) setLive({ text: t.out, emojiName: en.out, err: t.err || en.err });
        };
        run();
        const id = setInterval(run, 1000);
        return () => { alive = false; clearInterval(id); };
    }, [entry.text, entry.emojiName, isEvalText, isEvalEmoji]);
    ...
    return (
        <div className="rs-status-preview">
            ...
            <div className="rs-preview-info">
                <div className="rs-preview-label">PREVIEW{(isEvalText || isEvalEmoji) && <span style={{ color: live.err ? "#ef9a9a" : "#ffa726", marginLeft: 5 }}>· {live.err ? "eval error" : "live eval"}</span>}</div>
                ...
                <div className="rs-preview-row">
                    {entry.emojiId
                        ...
                        : live.emojiName
                            ? <span className="rs-preview-emoji">{live.emojiName}</span>
                            : null}
                    <span className="rs-preview-text" style={live.err ? { color: "#ef9a9a", fontFamily: "monospace", fontSize: 11 } : undefined}>{live.text || <em>No text set...</em>}</span>
                </div>
            </div>
        </div>
    );
...
function ClearAfterSelector({ value, onChange }: { value: number; onChange: (v: number) => void }) {
    ...
    return (
        <div style={{ margin: "4px 0" }}>
            ...
            <div style={{ fontSize: 10, color: "#5a4a7a", marginTop: 2, lineHeight: 1.4 }}>Wipes this status back to blank N minutes after it's set - leave 0 to keep it until the next rotation.</div>
        </div>
    );
}
...
function WeatherPanel({ forceUpdate }: { forceUpdate: () => void }) {
    const s = settings.store as any;
    const [city, setCity] = React.useState<string>(s.weatherCity ?? "");
    const [template, setTemplate] = React.useState<string>(s.weatherTemplate ?? "{emoji} {temp} · {city}");
    const [fetching, setFetching] = React.useState(false);
    const [localUnit, setLocalUnit] = React.useState<string>(s.weatherUnit ?? "C");

    const useF = localUnit === "F";
    const preview = wxCache ? wxBuildText(wxCache, template, useF) : null;

    const doFetch = async () => {
        const c = city.trim(); if (!c) return;
        setFetching(true);
        const r = await wxFetch(c);
        if (r) {
            wxCache = r;
            (window as any).__rsWx = { emoji: r.emoji, tempC: r.tempC, tempF: r.tempF, city: r.city, condition: r.condition };
        }
        setFetching(false); forceUpdate();
    };

    const toggleUnit = () => {
        const next = localUnit === "C" ? "F" : "C";
        s.weatherUnit = next; setLocalUnit(next); forceUpdate();
    };

    const rowStyle: React.CSSProperties = { display: "flex", alignItems: "center", gap: 6, margin: "5px 0" };
    const inputBase: React.CSSProperties = { background: "rgba(10,0,30,.7)", border: "1px solid rgba(100,181,246,.3)", borderRadius: 5, color: "#f0eaff", fontSize: 11, padding: "3px 7px", outline: "none" };
    const btnBase: React.CSSProperties = { padding: "3px 10px", borderRadius: 6, border: "1px solid rgba(100,181,246,.4)", background: "rgba(100,181,246,.1)", color: "#64b5f6", cursor: "pointer", fontSize: 11, fontWeight: 800 as const, outline: "none", flexShrink: 0 };

    return (
        <div style={{ border: "1px solid rgba(100,181,246,.28)", borderRadius: 9, padding: "9px 11px", marginTop: 6, background: "rgba(10,20,50,.45)" }}>
            <div style={{ fontSize: 10, fontWeight: 800, textTransform: "uppercase" as const, letterSpacing: ".7px", color: "#64b5f6", marginBottom: 7 }}>🌤 Weather Integration</div>
            <PanelToggle label="Weather Polling" description="Keep weather data cached - enables window.__rsWx in eval snippets"
                value={s.weatherEnabled} color="#64b5f6"
                onChange={v => { s.weatherEnabled = v; if (v) wxStart(); else wxStop(); forceUpdate(); }} />
            <div style={rowStyle}>
                <input value={city} onChange={e => setCity(e.target.value)}
                    onBlur={() => { s.weatherCity = city.trim(); }}
                    onKeyDown={(e: React.KeyboardEvent) => { if (e.key === "Enter") { s.weatherCity = city.trim(); void doFetch(); } }}
                    placeholder="City (e.g. Rome, New York)"
                    style={{ ...inputBase, flex: 1, fontSize: 12 }} />
                <button onClick={toggleUnit} style={btnBase}>°{localUnit}</button>
                <button onClick={doFetch} disabled={fetching || !city.trim()}
                    style={{ ...btnBase, opacity: (fetching || !city.trim()) ? .45 : 1 }}>
                    {fetching ? "…" : "Fetch"}
                </button>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 8, margin: "4px 0" }}>
                <span style={{ fontSize: 11, color: "#757575", flexShrink: 0 }}>Refresh every</span>
                <input type="number" defaultValue={s.weatherIntervalMin ?? 15}
                    onBlur={(e: React.FocusEvent<HTMLInputElement>) => { s.weatherIntervalMin = Math.max(1, parseInt(e.target.value) || 15); if (pluginActive && s.weatherEnabled) wxStart(); }}
                    style={{ ...inputBase, width: 52, textAlign: "center", fontFamily: "monospace" }} />
                <span style={{ fontSize: 11, color: "#757575" }}>min</span>
            </div>
            {wxCache && (
                <div style={{ fontSize: 10, color: "#64b5f6", margin: "5px 0", padding: "5px 9px", borderRadius: 5, background: "rgba(100,181,246,.07)", border: "1px solid rgba(100,181,246,.18)", lineHeight: 1.5 }}>
                    <b style={{ color: "#4fc3f7" }}>{wxCache.emoji} {wxCache.tempC}°C / {wxCache.tempF}°F</b> · {wxCache.city}
                    {wxCache.condition ? <span style={{ color: "#9e9e9e" }}> - {wxCache.condition}</span> : null}
                </div>
            )}
            <div style={{ borderTop: "1px solid rgba(100,181,246,.15)", marginTop: 8, paddingTop: 8 }}>
                <PanelToggle label="Standalone Weather Status" description="Set Discord status directly from weather (independent from rotator)"
                    value={s.weatherStandaloneEnabled} color="#4fc3f7" compact
                    onChange={v => { s.weatherStandaloneEnabled = v; forceUpdate(); }} />
                {s.weatherStandaloneEnabled && (
                    <div style={{ marginTop: 5, display: "flex", flexDirection: "column", gap: 5 }}>
                        <div style={{ fontSize: 10, color: "#9e9e9e" }}>
                            Tokens: <span style={{ fontFamily: "monospace", color: "#ffd580" }}>{"{emoji}  {temp}  {city}  {condition}"}</span>
                        </div>
                        <input value={template} onChange={e => setTemplate(e.target.value)}
                            onBlur={() => { s.weatherTemplate = template; }}
                            placeholder="{emoji} {temp} · {city}"
                            style={{ ...inputBase, width: "100%", boxSizing: "border-box" as const, fontFamily: "monospace" }} />
                        <div style={{ fontSize: 10, color: "#4fc3f7", padding: "4px 8px", borderRadius: 5, background: "rgba(79,195,247,.07)", border: "1px solid rgba(79,195,247,.15)" }}>
                            Preview: {preview ?? <em style={{ color: "#5a4a7a" }}>fetch weather first</em>}
                        </div>
                    </div>
                )}
            </div>
            <div style={{ fontSize: 10, color: "#5a4a7a", marginTop: 8, lineHeight: 1.6 }}>
                Eval tokens: <span style={{ fontFamily: "monospace", color: "#faa61a" }}>window.__rsWx?.emoji</span> · <span style={{ fontFamily: "monospace", color: "#faa61a" }}>tempC</span> · <span style={{ fontFamily: "monospace", color: "#faa61a" }}>tempF</span> · <span style={{ fontFamily: "monospace", color: "#faa61a" }}>city</span> · <span style={{ fontFamily: "monospace", color: "#faa61a" }}>condition</span>
            </div>
        </div>
    );
}

function PresetClearAfterRow({ preset, currentMs, onApply }: { preset: StatusPreset; currentMs: number; onApply: (ms: number) => void }) {
    const [rawMin, setRawMin] = React.useState(currentMs > 0 ? String(Math.round(currentMs / 60000)) : "");
    const commit = () => onApply(minutesToMs(rawMin));
    return (
        <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 3 }}>
            <span style={{ fontSize: 10, color: "#757575", flexShrink: 0 }}>Auto-clear all:</span>
            <input value={rawMin} onChange={e => setRawMin(e.target.value)} onBlur={commit}
                onKeyDown={(e: React.KeyboardEvent) => { if (e.key === "Enter") commit(); }}
                placeholder="0 = never"
                style={{ width: 64, background: "rgba(10,0,30,.7)", border: "1px solid rgba(124,77,255,.3)", borderRadius: 5, color: "#f0eaff", fontSize: 11, padding: "2px 6px", outline: "none", fontFamily: "monospace" }} />
            <span style={{ fontSize: 11, color: "#757575" }}>min</span>
            <button className="rs-btn" style={{ background: "rgba(124,77,255,.25)", fontSize: 10, padding: "2px 9px" }} onClick={commit}>Apply to "{preset.name}"</button>
        </div>
    );
}
...
function StatusTab({ forceUpdate }: { forceUpdate: () => void }) {
    ...
    const [presetEditIdx, setPresetEditIdx] = React.useState<number | null>(null);
    const [presetEditVal, setPresetEditVal] = React.useState("");
    const [presetDelConfirm, setPresetDelConfirm] = React.useState<string | null>(null);
    ...
    const { props: presetDProps, cls: presetCls } = useDrag((f, t) => {
        statusPresets = reorder(presets, f, t);
        saveData(); forceUpdate();
    });
    ...
    function renamePreset(i: number) {
        const old = presets[i];
        const name = presetEditVal.trim();
        setPresetEditIdx(null);
        if (!old || !name || name === old.name) return;
        if (presets.some((p, j) => j !== i && p.name.toLowerCase() === name.toLowerCase())) return;
        statusPresets = presets.map((p, j) => j === i ? { ...p, name } : p);
        statusEntries = list.map(e => e.preset === old.name ? { ...e, preset: name } : e);
        try {
            const sel: string[] = JSON.parse(settings.store.statusSelectedPresets || "[]");
            if (sel.includes(old.name)) settings.store.statusSelectedPresets = JSON.stringify(sel.map(s => s === old.name ? name : s));
        } catch {}
        if (filterPreset === old.name) setFilterPreset(name);
        if (draftPreset === old.name) setDraftPreset(name);
        if (editPreset === old.name) setEditPreset(name);
        saveData(); forceUpdate();
    }
    ...
    return (
        <div>
            <div className="rs-settings-panel">
                ...
                <PanelToggle label="Randomize" description="Pick randomly instead of cycling in order" value={settings.store.statusRandomize} color={C.status}
            ...
            <WeatherPanel forceUpdate={forceUpdate} />
            <div className="rs-divider" style={{ margin: "8px 0" }} />
            ...
            <textarea className="rs-status-draft" value={draft} onChange={e => setDraft(e.target.value)}
                placeholder="Status text... or Discord emoji <:name:id> + text" rows={2}
                onKeyDown={(e: React.KeyboardEvent<HTMLTextAreaElement>) => { if (e.key === "Enter") { e.preventDefault(); if (e.ctrlKey) add(); } }} />
            ...
            {presets.length > 0 && (
                <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 5 }}>
                    ...
                    <select value={draftPreset} onChange={e => setDraftPreset(e.target.value)}
                        ...
                        {presets.map(p => <option key={p.id} value={p.name} style={{ color: "#000" }}>{p.name}</option>)}
                    </select>
                </div>
            )}
            ...
            <div className="rs-hint" style={{ marginBottom: 5 }}>
                Group entries to cycle only a chosen subset (Run Mode above) or to share a bulk auto-clear timer. Drag ⠿ to reorder, click name to filter the list below, ✎ to rename.
            </div>
            ...
            {presets.length === 0 && <div className="rs-empty" style={{ marginBottom: 6 }}>No presets yet. Add one below.</div>}
            <div style={{ display: "flex", flexDirection: "column", gap: 4, marginBottom: 6 }}>
                {presets.map((p, i) => {
                    const cnt = list.filter(e => e.preset === p.name).length;
                    const curMs = list.find(e => e.preset === p.name)?.clearAfter ?? 0;
                    return (
                        <div key={p.id} {...presetDProps(i)} className={presetCls(i, "rs-item rs-item-compact")}
                            style={{ flexDirection: "column", alignItems: "stretch" }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                                <span className="rs-drag">⠿</span>
                                {presetEditIdx === i ? (
                                    <input autoFocus className="rs-item-input" maxLength={32} value={presetEditVal}
                                        onChange={e => setPresetEditVal(e.target.value)}
                                        onKeyDown={(e: React.KeyboardEvent) => { if (e.key === "Enter") renamePreset(i); if (e.key === "Escape") setPresetEditIdx(null); }}
                                        onBlur={() => renamePreset(i)} />
                                ) : (
                                    <span className="rs-preset-tag" style={{ cursor: "pointer", flex: 1, opacity: filterPreset === p.name ? 1 : .6 }}
                                        onClick={() => setFilterPreset(filterPreset === p.name ? null : p.name)}
                                        onDoubleClick={() => { setPresetEditIdx(i); setPresetEditVal(p.name); }}>
                                        {p.name} ({cnt})
                                    </span>
                                )}
                                <button className="rs-edit-btn" onClick={() => { setPresetEditIdx(i); setPresetEditVal(p.name); }} title="Rename preset">&#9998;</button>
                                <button className="rs-del-btn" onClick={() => setPresetDelConfirm(p.id)} title="Delete preset">✕</button>
                            </div>
                            {presetDelConfirm === p.id
                                ? <ConfirmBox msg={`Delete preset "${p.name}"?`} onConfirm={() => { removePreset(p.id); setPresetDelConfirm(null); }} onCancel={() => setPresetDelConfirm(null)} />
                                : <PresetClearAfterRow preset={p} currentMs={curMs} onApply={ms => applyPresetClearAfter(p.name, ms)} />}
                        </div>
                    );
                })}
            </div>
            ...
            <div className="rs-hint" style={{ marginTop: 6 }}>
                Ctrl+Enter to add · Drag ⠿ to reorder · Click text to rename · <b style={{ color: settings.store.statusEnabled ? C.enabled : "#757575" }}>{settings.store.statusEnabled ? "Enabled" : "Disabled"}</b>, swapping status text every <b style={{ color: C.data }}>{settings.store.statusIntervalSeconds}s</b> {settings.store.statusRandomize ? <>in <b style={{ color: "#ab47bc" }}>random</b> order</> : <>in <b style={{ color: "#ab47bc" }}>sequential</b> order</>}. The blue badge on an entry is its own auto-clear timer (wipes that one status after N minutes); the "Auto-clear all" box on a preset sets that timer for every entry in the group at once.
            </div>
        </div>
    );
}
    ...
    return (
        <div>
            <div className="rs-settings-panel">
                ...
                <PanelToggle label="Randomize" description="Pick clan randomly instead of in order" value={settings.store.clanRandomize} color={C.clan}
                    onChange={v => { settings.store.clanRandomize = v; }} />
                <PanelToggle label="Auto-Detect" description="Automatically cycle through all your joined servers" value={settings.store.clanAutoDetect} color={C.clan}
...
function bcrNitroBlocked(): boolean {
    return !!settings.store.bannerNitroCheck && arHasNitro();
}
...
async function arApplyAvatar(entry: AvatarEntry): Promise<void> {
    const run = arApplyPromise.then(() => arApplyAvatarExec(entry));
    arApplyPromise = run.catch(() => {});
    return run;
}

async function arApplyAvatarExec(entry: AvatarEntry): Promise<void> {
    try {
        const isGif = arIsGif(entry.data);
        if (isGif) {
            const uploadData = arStampGif(entry.data);
            try {
...
function arGetActive(): AvatarEntry[] {
    ...
    const noNitroBlock = settings.store.avatarNitroCheck && !arHasNitro();
    return arAvatars.filter(a => {
        const ext = arGetExt(a.data);
        if (excl.includes(ext)) return false;
        if (ext === "gif" && noNitroBlock) return false;
        return true;
    });
}
...
function arSchedule() {
    ...
    const ms = Math.max(1, settings.store.avatarIntervalSeconds || AR_DEFAULT_S) * 1000;
    arRotatorTimer = setTimeout(arRotateNext, netGuardMultiplier > 1 ? ms * netGuardMultiplier : ms);
}
            ...
            <div className="ar-card">
                ...
                <div className="ar-card-row">
                    ...
                    <div onClick={() => {
                        ...
                        if (running) { if (n) startAvatarNitroWatcher(); else stopAvatarNitroWatcher(); }
                        forceUpdate();
            ...
            {!stopped && (
                <div style={{ display: "flex", gap: 7, alignItems: "center", flexWrap: "wrap" as const, marginBottom: 8 }}>
                    ...
                    <button onClick={() => { doGlobalStop(null); forceUpdate(); }}
                        style={{ padding: "4px 12px", borderRadius: 6, fontSize: 12, fontWeight: 700, cursor: "pointer", border: "1px solid rgba(171,71,188,.45)", background: "rgba(171,71,188,.14)", color: "#ce93d8" }}>
                        ⏹ Stop ∞
                    </button>
                </div>
            )}
...
function NetGuardCard({ forceUpdate }: { forceUpdate: () => void }) {
    const enabled = !!(settings.store as any).netGuardEnabled;
    const rtt = _netGetRtt();
    const th = Math.max(50, Number((settings.store as any).netGuardRttThreshold) || 200);
    const [thVal, setThVal] = React.useState(String(th));

    let statusColor = "#66bb6a";
    let statusLabel = "OK";
    if (rtt === 0) { statusColor = "#9e9e9e"; statusLabel = "unknown (no API)"; }
    else if (netGuardPaused) { statusColor = "#ef9a9a"; statusLabel = `PAUSED - ${rtt}ms`; }
    else if (netGuardMultiplier > 1) { statusColor = "#faa61a"; statusLabel = `SLOW ×${netGuardMultiplier} - ${rtt}ms`; }
    else { statusColor = "#66bb6a"; statusLabel = `${rtt}ms`; }

    const commitTh = (v: string) => {
        const n = Math.max(50, parseFloat(v) || 200);
        (settings.store as any).netGuardRttThreshold = n;
        setThVal(String(n));
        if (enabled) _netApply();
    };

    return (
        <div className="rs-data-card" style={{ borderColor: "rgba(38,198,218,.28)" }}>
            <div className="rs-data-title" style={{ color: "#26c6da" }}>Network Guard</div>
            <div className="rs-data-desc" style={{ marginBottom: 7 }}>Slows (×3–×5) or pauses rotators when RTT is high - reduces rate-limit spam on bad connections.</div>
            <PanelToggle label="Network Guard" color="#26c6da"
                description={enabled ? `Slow ×3 above threshold · Slow ×5 above 1.5× · Pause above 2.5×` : "All rotators run at full speed regardless of connection"}
                value={enabled}
                onChange={v => {
                    (settings.store as any).netGuardEnabled = v;
                    if (v) startNetGuard();
                    else { stopNetGuard(); if (!isManualStop && !wasInvisible && pluginActive) startAllRotators(); }
                    forceUpdate();
                }} />
            {enabled && (
                <div style={{ marginTop: 7, display: "flex", flexDirection: "column", gap: 5 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        <span style={{ fontSize: 11, color: "#9e9e9e", flexShrink: 0 }}>RTT threshold:</span>
                        <input type="number" min={50} value={thVal}
                            onChange={e => setThVal(e.target.value)}
                            onBlur={e => commitTh(e.target.value)}
                            onKeyDown={(e: React.KeyboardEvent) => { if (e.key === "Enter") commitTh((e.target as HTMLInputElement).value); }}
                            style={{ width: 62, background: "rgba(10,0,30,.7)", border: "1px solid rgba(38,198,218,.35)", borderRadius: 5, color: "#f0eaff", fontSize: 12, padding: "2px 7px", outline: "none", textAlign: "center" }} />
                        <span style={{ fontSize: 11, color: "#9e9e9e" }}>ms</span>
                        <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 5 }}>
                            <div style={{ width: 7, height: 7, borderRadius: "50%", background: statusColor, flexShrink: 0 }} />
                            <span style={{ fontSize: 11, color: statusColor, fontFamily: "monospace" }}>{statusLabel}</span>
                        </div>
                    </div>
                    <div style={{ fontSize: 10, color: "#5a8a9a", lineHeight: 1.5 }}>
                        {`Slow ×3 at >${th}ms · Slow ×5 at >${Math.round(th * 1.5)}ms · Pause at >${Math.round(th * 2.5)}ms`}
                    </div>
                </div>
            )}
        </div>
...
function ConsoleCleanerCard({ forceUpdate }: { forceUpdate: () => void }) {
    const enabled = !!(settings.store as any).consoleCleanerEnabled;
    const th = Math.max(50, Number((settings.store as any).consoleCleanThreshold) || 500);
    const [thVal, setThVal] = React.useState(String(th));

    const commitTh = (v: string) => {
        const n = Math.max(50, parseFloat(v) || 500);
        (settings.store as any).consoleCleanThreshold = n;
        setThVal(String(n));
    };
    const pct = Math.min(100, Math.round((_consoleErrCount / th) * 100));
    const barColor = pct > 80 ? "#ef9a9a" : pct > 50 ? "#faa61a" : "#66bb6a";

    return (
        <div className="rs-data-card" style={{ borderColor: "rgba(102,187,106,.28)" }}>
            <div className="rs-data-title" style={{ color: "#66bb6a" }}>Console Cleaner</div>
            <div className="rs-data-desc" style={{ marginBottom: 7 }}>
                Clears the console when JS errors/warns (console.error/warn + unhandled rejections) exceed the threshold - prevents Discord crash from log overflow.
            </div>
            <PanelToggle label="Auto-clear console" color="#66bb6a"
                description={enabled ? `Clears at ${th} JS errors/warns` : "Console grows unbounded"}
                value={enabled}
                onChange={v => {
                    (settings.store as any).consoleCleanerEnabled = v;
                    if (v) startConsoleCleaner(); else stopConsoleCleaner();
                    forceUpdate();
                }} />
            {enabled && (
                <div style={{ marginTop: 7, display: "flex", flexDirection: "column", gap: 6 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        <span style={{ fontSize: 11, color: "#9e9e9e", flexShrink: 0 }}>Clear at:</span>
                        <input type="number" min={50} value={thVal}
                            onChange={e => setThVal(e.target.value)}
                            onBlur={e => commitTh(e.target.value)}
                            onKeyDown={(e: React.KeyboardEvent) => { if (e.key === "Enter") commitTh((e.target as HTMLInputElement).value); }}
                            style={{ width: 70, background: "rgba(10,0,30,.7)", border: "1px solid rgba(102,187,106,.35)", borderRadius: 5, color: "#f0eaff", fontSize: 12, padding: "2px 7px", outline: "none", textAlign: "center" }} />
                        <span style={{ fontSize: 11, color: "#9e9e9e" }}>JS errors</span>
                        <button onClick={() => { console.clear(); _consoleErrCount = 0; forceUpdate(); }}
                            style={{ marginLeft: "auto", padding: "3px 10px", borderRadius: 5, fontSize: 11, fontWeight: 700, cursor: "pointer", border: "1px solid rgba(102,187,106,.4)", background: "rgba(102,187,106,.12)", color: "#66bb6a" }}>
                            Clear Now
                        </button>
                    </div>
                    <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
                        <div style={{ display: "flex", justifyContent: "space-between" }}>
                            <span style={{ fontSize: 10, color: "#9e9e9e" }}>JS errors since last clear</span>
                            <span style={{ fontSize: 10, fontFamily: "monospace", color: barColor }}>{_consoleErrCount} / {th}</span>
                        </div>
                        <div style={{ height: 4, borderRadius: 2, background: "rgba(255,255,255,.08)", overflow: "hidden" }}>
                            <div style={{ height: "100%", width: `${pct}%`, background: barColor, transition: "width .3s, background .3s", borderRadius: 2 }} />
                        </div>
                    </div>
                </div>
            )}
...
function DataTab({ forceUpdate }: { forceUpdate: () => void }) {
    ...
    return (
        <div>
            ...
            <NetGuardCard forceUpdate={forceUpdate} />
            <ConsoleCleanerCard forceUpdate={forceUpdate} />
