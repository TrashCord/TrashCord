...
import { PresenceStore, Tooltip, useMemo, UserStore, useStateFromStores } from "@webpack/common";
...
const PlatformIcon = ({ platform, status, small }: { platform: string; status: string; small: boolean; }) => {
...
function sortSessions(sessions: Record<string, Session>) {
    return Object.values(sessions).sort(({ status: a }, { status: b }) => {
        if (a === b) return 0;
...
function shallowEqualStatus(a: Record<string, string> | null | undefined, b: Record<string, string> | null | undefined) {
    if (a === b) return true;
    if (!a || !b) return false;
...
function updateOwnStatus() {
    ...
    const ownStatus = sortedSessions.reduce((acc, curr) => {
...
const PlatformIndicator = ({ user, isProfile, isMessage, isMemberList }: PlatformIndicatorProps) => {
    ...
    const small = !!(isProfile || isMemberList);

    const className = useMemo(
        () => classes("vc-platform-indicator", isProfile && "vc-platform-indicator-profile", isMessage && "vc-platform-indicator-message", isMemberList && "vc-platform-indicator-memberlist"),
        [isProfile, isMessage, isMemberList]
    );

    const icons = useMemo(() => {
        if (!status) return [];
        return Object.entries(status).map(([platform, platformStatus]) => (
            <PlatformIcon
                key={platform}
    ...
    if (user == null || (user.bot && !settings.store.showBots) || !status || !icons.length) return null;
...
export default definePlugin({
    ...
    managedStyle,
    tags: ["Appearance"],
    authors: [Devs.kemo, Devs.TheSun, Devs.Nuckyz, Devs.Ven, EquicordDevs.neoarz, { name: "zfrancesck1", id: 456195985404592149n }],
    isModified: true,
    settings,
    start() {
        SessionsStore.addChangeListener(updateOwnStatus);
        updateOwnStatus();
    },
    stop() {
        SessionsStore.removeChangeListener(updateOwnStatus);
    },
