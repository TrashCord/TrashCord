...
const PANEL_BUTTON_SETTINGS_KEYS = ["followUserId", "followedUsername"];
const MODAL_SETTINGS_KEYS = ["followUserId", "followedUsername"];
...
function FollowUserModal({ modalProps }: { modalProps: RenderModalProps; }) {
    ...
    const { followUserId, followedUsername } = settings.use(MODAL_SETTINGS_KEYS);
...
function openFollowModal() {
    openModal(props => <FollowUserModal modalProps={props} />);
}

function PanelButton({ iconForeground, hideTooltips, nameplate }: UserAreaRenderProps) {
    const { followUserId, followedUsername } = settings.use(PANEL_BUTTON_SETTINGS_KEYS);

    const isFollowing = !!followUserId;
    const name = isFollowing ? (UserStore.getUser(followUserId)?.username ?? followedUsername) : followedUsername;

    const cacheKey = iconForeground + "|" + hideTooltips + "|" + (nameplate != null) + "|" + isFollowing + "|" + name;
    const cache = React.useRef<{ key: string; node: React.ReactNode; }>();
...
export const FollowPanelButton: UserAreaButtonFactory = props => settings.store.showPanelButton ? <PanelButtonBoundary {...props} /> : null;
export { FollowIcon };