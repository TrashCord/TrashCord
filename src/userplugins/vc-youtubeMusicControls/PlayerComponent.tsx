...
function Controls() {
    const [isPlaying, isShuffled, repeat] = useStateFromStores<
        [boolean, boolean, Repeat]
    >([YoutubeMusicStore], () => [
        YoutubeMusicStore.isPlaying,
        YoutubeMusicStore.isShuffled,
        YoutubeMusicStore.repeat,
    ]);

    const { showYoutubeMusicLyrics } = settings.use(["showYoutubeMusicLyrics"]);
        ...
        <Flex className={cl("button-row")} style={{ gap: 0 }}>
            <Button
                className={classes(
                    cl("button"),
                    cl("shuffle"),
                    isShuffled ? cl("shuffle-on") : cl("shuffle-off"),
                )}
            ...
            <Button
                className={classes(
                    cl("button"),
                    cl("lyrics"),
                    showYoutubeMusicLyrics ? cl("lyrics-on") : cl("lyrics-off"),
                )}
                onClick={() =>
                (settings.store.showYoutubeMusicLyrics =
...
function YtmSeekBar() {
    ...
    const [storePosition, isSettingPosition, isPlaying] = useStateFromStores(
        ...
        () => [
            ...
            YoutubeMusicStore.isSettingPosition,
            YoutubeMusicStore.isPlaying,
        ],
    );
...
function AlbumContextMenu({ track }: { track: Song; }) {
    const [volume, muted] = useStateFromStores([YoutubeMusicStore], () => [
        YoutubeMusicStore.volume,
        YoutubeMusicStore.muted,
    ]);
