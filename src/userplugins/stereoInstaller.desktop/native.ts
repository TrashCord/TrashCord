...
let hasRunStartupChecksThisBoot = false;

export async function shouldRunStartupChecks(_: IpcMainInvokeEvent): Promise<NativeResult<boolean>> {
    if (hasRunStartupChecksThisBoot) return ok(false, []);
    hasRunStartupChecksThisBoot = true;
    return ok(true, []);
}
...
const LOG_ROTATE_CHECK_BYTES = 64 * 1024;

function rotateLogFileIfNeeded(pathValue: string): void {
    try {
        if (!existsSync(pathValue) || statSync(pathValue).size < LOG_ROTATE_CHECK_BYTES) return;

        const lines = readFileSync(pathValue, "utf8").split(/\r?\n/).filter(Boolean);
        if (lines.length <= MAX_VISIBLE_LOG_LINES) return;

        writeFileSync(pathValue, `${lines.slice(-MAX_VISIBLE_LOG_LINES).join("\n")}\n`, "utf8");
    } catch { }
}
...
function appendLogLine(line: string): void {
    for (const pathValue of logPaths()) {
        try {
            ...
            rotateLogFileIfNeeded(pathValue);
            appendFileSync(pathValue, `${line}\n`, "utf8");
        } catch { }
    }
}

function pendingRestartMarkerPath(): string {
    return join(hubDataDir(), "pending-restart.flag");
}

function markPendingWorkerRestart(): void {
    try {
        mkdirSync(dirname(pendingRestartMarkerPath()), { recursive: true });
        writeFileSync(pendingRestartMarkerPath(), String(Date.now()), "utf8");
    } catch { }
}

function clearLogsUnlessWorkerRestartPending(): void {
    const markerPath = pendingRestartMarkerPath();
    ...
    for (const pathValue of logPaths()) {
        try {
            mkdirSync(dirname(pathValue), { recursive: true });
            writeFileSync(pathValue, "", "utf8");
        } catch { }
    }
}

clearLogsUnlessWorkerRestartPending();
