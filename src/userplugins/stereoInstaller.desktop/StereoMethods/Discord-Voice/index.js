...
if (isElectronRenderer) {
    try {
        ...
        dataDirectory = fileManager.getModuleDataPathSync
            ? path.join(fileManager.getModuleDataPathSync(), 'discord_voice')
            : null;
    }
    catch (e) {
...
if (isLogDirAvailable != null) {
    logDirectory = discordNative.fileManager.getAndCreateLogDirectorySync() ?? undefined;
}
else {
...
function bindConnectionInstance(instance) {
    return {
        ...
        prepareSecureFramesTransition: (transitionId, version, callback) => instance.prepareSecureFramesTransition(transitionId, version, callback),
        ...
        getMLSPairwiseFingerprint: (version, userId, callback) => instance.getMLSPairwiseFingerprint(version, userId, callback),
        ...
        setRemoteUserCanHavePriority: (userId, canHavePriority) => instance.setRemoteUserCanHavePriority(userId, canHavePriority),
...
const setAudioSubsystemInternal = function (subsystem, forceRestart) {
    ...
    if (isElectronRenderer) {
        if (forceRestart) {
            ...
            void discordNative.app.relaunch();
        }
        else {
...
let activeSinksChangeCallback = null;
