...
function OpenStalkerButton() {
    return (
        <HeaderBarIcon
            className="firestoker-toolbox-btn"
...
export default definePlugin({
    name: "Fire Stoker",
    description: 
    "Track presence, voice activity, profile changes and user history in real time.",
    authors: [{
        id: 1113803832191885383n,
        name: "Fire,Sky"
    }],
    tags: ["Activity", "Voice", "Chat", "Utility"],
    ...
    StalkerOverviewComponent: ErrorBoundary.wrap(({ userId, Divider }: { userId: string; Divider?: any; }) => {
        ...
        if (Divider !== undefined) {
            return (
                <div className="firestoker-overview-card">
                    <div className="firestoker-overview-card-header">FIRE STOKER</div>
                    <div className="firestoker-overview-card-body">
                        <div className="firestoker-overview-stat">
                            <span className="firestoker-overview-stat-label">Status:</span>
                            <span className="firestoker-overview-stat-value">{text}</span>
                        </div>
                        {!online && (
                            <div className="firestoker-overview-stat">
                                <span className="firestoker-overview-stat-label">Last Seen:</span>
                                <span className="firestoker-overview-stat-value">{lastSeenText}</span>
                            </div>
                        )}
                        <div className="firestoker-overview-stat">
                            <span className="firestoker-overview-stat-label">Total Logs:</span>
                            <span className="firestoker-overview-stat-value">{totalLogs}</span>
                        </div>
                    </div>
                </div>
            );
        ...
        return (
            <Section title="Stalker Overview">
                <div className="firestoker-overview-text">{text}</div>
            </Section>
        );
    }),
    flux: {
            ...
            try {
                ...
                if (userConfig.notifyMessages) {
                    try {
                        ...
                        showNotification({
                            title: `Fire Stoker • Message Detected`,
    ...
    try {
        const states = payload?.voiceStates ?? payload?.voice_states;

        if (!Array.isArray(states)) return;

        const trackedUsers = getWhitelistedIds();
        ...
        for (const state of states) {
            ...
            const existing =
                activeVoiceSessions.get(userId);

            if (!existing && channelId) {
                voiceJoinTimestamps.set(
                    userId,
                    Date.now()
                );
                ...
                activeVoiceSessions.delete(
                    userId
                );

                voiceJoinTimestamps.delete(
                    userId
                );
            }
    ...
    addIconToToolBar(e: { toolbar: any; }) {
        ...
        e.toolbar = [
            <ErrorBoundary noop={true} key="firestoker-button">
                <OpenStalkerButton />
            </ErrorBoundary>,
            e.toolbar,
        ];
                                    ...
                                    if (userConfig.logProfileChanges) {
                                        ...
                                        if (userConfig.notifyProfileChanges) {
                                            try {
                                                ...
                                                showNotification({
                                                    title: `Fire Stoker • ${user.username}`,
                                                    body: changeLabels.join(", "),
...
const contextMenuPatch: NavContextMenuPatchCallback = (children, props) => {
    ...
    if (!children.some(child => child?.props?.id === "firestoker-v1")) {
        const userId = props.user.id;
        const isWhitelisted = isInWhitelist(userId);
        const menuItems = [
            <Menu.MenuSeparator key="firestoker-sep" />,
            <Menu.MenuItem
                key="firestoker-item"
                id="firestoker-v1"
                label={isWhitelisted ? "Remove From Fire Stoker" : "Track With Fire Stoker"}
        ...
        if (isWhitelisted) {
            menuItems.push(
                <Menu.MenuItem
                    id="firestoker-view-log"
                    label="View Fire Stoker History"
