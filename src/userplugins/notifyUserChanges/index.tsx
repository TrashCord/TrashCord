...
const PlatformIndicator = ({ user, wantMargin = true, wantTopMargin = false, small = false, style = {} }: PlatformIndicatorProps) => {
    ...
    const isOwnUser = !!user && user.id === currentUserId;
    const ownSessions = isOwnUser ? (SessionsStore.getSessions() as Sessions) : null;

    const ownStatus = React.useMemo(() => {
        if (!ownSessions || typeof ownSessions !== "object") return null;
        const sortedSessions = Object.values(ownSessions).sort(({ status: a }, { status: b }) => {
            if (a === b) return 0;
    ...
    const status = isOwnUser
        ? ownStatus as Record<Platform, string> | null
        : PresenceStore.getState()?.clientStatuses?.[user.id] as Record<Platform, string>;
...
let cachedUserIdSet: Set<string> = new Set();

function getFollowedUserIdSet(): Set<string> {
    const raw = settings.store.userIds;
    if (raw !== cachedUserIdsRaw) {
        cachedUserIdsRaw = raw;
        cachedUserIdSet = new Set(getUserIdList());
    }
    return cachedUserIdSet;
}
...
const UserContext: NavContextMenuPatchCallback = (children, { user }: UserContextProps) => {
    ...
    const isNotifyOn = getFollowedUserIdSet().has(user.id);
...
export default definePlugin({
    ...
    flux: {
        VOICE_STATE_UPDATES({ voiceStates }: { voiceStates: VoiceState[]; }) {
            ...
            const followedIds = getFollowedUserIdSet();
            for (const { userId, channelId, oldChannelId } of voiceStates) {
                if (!followedIds.has(userId)) continue;
